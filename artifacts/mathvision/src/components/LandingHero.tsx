...
function NavAnchorRow({
  ...
  const navBg = useTransform(scrollProgress, [0, 0.7, 1], ["transparent", "rgba(255, 248, 242, 0.95)", "rgba(255, 248, 242, 0.95)"]);
  const navBorder = useTransform(scrollProgress, [0, 0.7, 1], ["transparent", "var(--site-border)", "var(--site-border)"]);
  const navBlur = useTransform(scrollProgress, [0, 0.7, 1], ["none", "blur(20px)", "blur(20px)"]);
  const navHeight = useTransform(scrollProgress, [0, 0.7, 1], ["auto", "56px", "56px"]);
  const navPadding = useTransform(scrollProgress, [0, 0.7, 1], ["20px 32px", "0 32px", "0 32px"]);
  const logoFontSize = useTransform(scrollProgress, [0, 0.7, 1], ["1.25rem", "1.15rem", "1.15rem"]);
  const logoLetterSpacing = useTransform(scrollProgress, [0, 0.7, 1], ["-0.02em", "-0.03em", "-0.03em"]);
        ...
        <Show when="signed-out">
          <button
            ...
            style={{
              fontSize: "0.88rem",
              fontWeight: 700,
              background: signInHovered ? "var(--site-text)" : "var(--site-text)",
              color: "var(--site-bg)",
              border: "none",
              padding: "8px 20px",
              borderRadius: "8px",
...
function ScrollIndicator({ scrollProgress }: { scrollProgress: ReturnType<typeof useMotionValue<number>> }) {
  const opacity = useTransform(scrollProgress, [0, 0.15], [1, 0]);
  const y = useSpring(useTransform(scrollProgress, [0, 0.5], [0, 8]), { stiffness: 200, damping: 20 });
  ...
  return (
    ...
    >
      ...
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="var(--site-text-muted)" strokeWidth="1.5">
...
export function LandingHero() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const progress = Math.min(Math.max(-wrapperRect.top / vh, 0), 1);
  ...
  const contentTranslateY = useTransform(scrollYProgress, [0, 0.5], [0, -60]);

  const demoOpacity = useTransform(scrollYProgress, [0, 0.35, 0.65], [1, 1, 0]);
  const demoTranslateY = useTransform(scrollYProgress, [0, 0.5, 1], [0, 0, 120]);
  const demoScale = useTransform(scrollYProgress, [0, 1], [1, 0.88]);
        ...
        >
          <motion.div
            style={{
              ...
              transform: `translateY(${contentTranslateY.get()}px) scale(${contentScale.get()})`,
            }}
          >
            <h1
              style={{
          ...
          <motion.div
            style={{
              ...
              maxWidth: "900px",
              opacity: demoOpacity,
              transform: `translateY(${demoTranslateY.get()}px) scale(${demoScale.get()})`,
            }}
          >
            <CircleToSineDemo />
          </motion.div>
